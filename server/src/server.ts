...
expressApp.post("/viz", (req, res) => {
  ...
  plotConfigs.forEach((plotConfig) => {
    ...
    if (labels.has(plotConfig.label)) {
      ...
      let uniqueLabel = plotConfig.label + "-" + (labels.get(plotConfig.label) + 1);
...
expressApp.post("/workfreq", async (req, res) => {
  ...
  let associatedPlotConfig: PlotObj = Array.from(configurationCache).find(
    (cachedPlotConfig) =>    
      ((cachedPlotConfig && cachedPlotConfig[1]?.uniqueLabel) ?? cachedPlotConfig[1]?.label) === params.label
  )?.[1];    
  ...
  let workTableFormPairs = new Map(
    Array.from(associatedPlotConfig.Work).map((tableKey: [string, DynamicFormEntry]) => [
      tableKey[1].label,
      tableKey[1].value,
    ])
  );
  ...
  let selectedAuthors =
    associatedPlotConfig.authors?.length > 0 ?? undefined
      ? Array.from(associatedPlotConfig.authors).map((authorObj) => authorObj?.label)
      : undefined;
